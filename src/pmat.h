...
public:
  ...
  Pmat(int M, int N) : array(M), M(M), N(N) {
    for (int i = 0; i < M; i++)
      array[i].resize(N);
  }

  Pmat(int M, int N, T v) : array(M), M(M), N(N) {
    for (int i = 0; i < M; i++)
      array[i].resize(N, v);
  }

  template <class T2> Pmat(const Pmat<T2> &ma) {
  ...
  template <class T2> Pmat(const vector<vector<T2>> &ma) {
	M = ma.size();
	N = ma[0].size();
    ...
    template <class T2> Pmat(const vector<Pvec<T2>> &ma) {
		this->array = ma;
		M = ma.size();
		N = ma[0].size();
  }
  ...
  template <class T2> Pmat<T> &operator=(const vector<vector<T2>> &ma) {
    array.resize(ma.size());
    for (int i = 0; i < array.size(); ++i) {
  ...
  Pmat<double> toDouble() { return Pmat<double>(*this); }
